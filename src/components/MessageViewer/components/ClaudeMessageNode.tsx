...
const CAPTURE_HOVER_BG =
  "group-hover/capture:bg-red-500/5 group-hover/capture:ring-1 group-hover/capture:ring-red-500/20";
...
export const ClaudeMessageNode = React.memo(
  ({
    ...
    const selectionCursor =
      isCaptureMode && onRangeSelect ? "cursor-crosshair select-none" : "";

    // Capture mode hide button - appears on hover
    const CaptureHideButton =
    ...
    if (message.type === "system") {
      const systemContent = Array.isArray(message.content)
        ? message.content
        : (extractClaudeMessageContent(message) ?? undefined);
      return (
        <ExpandKeyProvider value={message.uuid}>
          <div
            data-message-uuid={message.uuid}
    ...
    const hasInlineToolResult =
      ...
      message.content.some(isToolResultContent);
              ...
              {message.content && Array.isArray(message.content) && (
                <div className="mb-2">
                  <ClaudeContentArrayRenderer
                    ...
                    skipThinking={
                      message.type === "assistant" &&
                      !messageFilter.contentTypes.thinking
                    }
                    skipCommands={
                      message.type === "assistant" &&
