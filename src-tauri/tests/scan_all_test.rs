...
// 运行: cargo test test_scan_all_projects -- --nocapture --test-threads=1
...
mod integration_tests {
    ...
    async fn test_scan_all_projects_full() {
        ...
        if detected.is_empty() {
            eprintln!("No providers detected in this environment, skipping probe");
            return;
        }
    ...
    async fn test_antigravity_project_stats_summary_returns_tools_for_logged_project() {
        ...
        let Some(project) = providers::antigravity::scan_projects()
            .expect("scan_projects failed")
            .into_iter()
            .next()
        else {
            println!("No antigravity projects found; skipping test");
            return;
        };
