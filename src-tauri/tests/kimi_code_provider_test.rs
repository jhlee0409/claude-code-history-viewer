...
fn kimi_code_provider_scans_projects_from_sessions_tree() {
    ...
    assert_eq!(project.message_count, 7);
...
fn kimi_code_provider_loads_messages_without_internal_events() {
    ...
    // tool.result becomes a standalone tool message.
    assert_eq!(messages[1].message_type, "tool");
    let tool_block = &messages[1].content.as_ref().unwrap()[0];
    ...
    // timestamped by its step.end.
    assert_eq!(messages[2].message_type, "assistant");
    assert_eq!(messages[2].timestamp, "2026-07-21T02:18:24+00:00");
    let blocks = messages[2].content.as_ref().unwrap().as_array().unwrap();
    ...
    assert_eq!(messages[4].timestamp, "2026-07-21T02:18:28+00:00");
    assert_eq!(
