...
`/api/*` 엔드포인트는 Bearer 토큰이 필요합니다. 토큰은 시작할 때 자동 생성되어 로컬에 저장되며 stderr에는 짧은 미리보기만 출력됩니다.
...
`--no-auth`는 `127.0.0.1` 또는 `localhost` 같은 loopback 호스트에 바인드할 때만 기본으로 허용됩니다. 네트워크에서 접근 가능한 호스트에는 token 인증을 유지하거나, 의도적으로 인증 없는 네트워크 접근을 원할 때만 `--allow-unsafe-no-auth`를 추가하세요.
