...
すべての`/api/*`エンドポイントはトークン認証で保護されます。トークンはサーバー起動時に自動生成されstderrに出力されます。

- **ブラウザアクセス**: 起動時に表示される`?token=...`URLを使用。ブラウザはこれをHttpOnly cookieに交換し、ログイン後にスクリプトから読めるローカルトークンを削除します。
